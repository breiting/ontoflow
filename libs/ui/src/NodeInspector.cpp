...
InspectorAction NodeInspector::Draw(domain::Registry& reg) {
    if (!m_IsOpen) return InspectorAction::None;
    ...
    if (ImGui::BeginPopupModal("Node Inspector", &p_open, flags)) {
        ...
        if (space > 0) ImGui::Dummy(ImVec2(space, 0));
        ImGui::SameLine();
        ...
        if (ImGui::Button("Save", ImVec2(70, 30)) || (ImGui::IsKeyPressed(ImGuiKey_Enter) && !ImGui::IsAnyItemActive())) {
             action = InspectorAction::Save;
             ImGui::CloseCurrentPopup();
             m_IsOpen = false;
        }
...
void NodeInspector::DrawPinsSection() {
    if (m_TempNode.inputs.empty() && m_TempNode.outputs.empty()) return;
...
void NodeInspector::DrawPinEditor(domain::Pin& pin) {
    switch (pin.type) {
        case domain::PinType::DOUBLE: {
            double v = 0.0;
            if (auto* pv = std::get_if<double>(&pin.value)) v = *pv;
        ...
        case domain::PinType::INT: {
            int v = 0;
            if (auto* pv = std::get_if<int>(&pin.value)) v = *pv;
        ...
        case domain::PinType::BOOL: {
            bool v = false;
            if (auto* pv = std::get_if<bool>(&pin.value)) v = *pv;
        ...
        case domain::PinType::STRING: {
            std::string v = "";
            if (auto* pv = std::get_if<std::string>(&pin.value)) v = *pv;
        ...
        case domain::PinType::VEC3: {
            glm::vec3 v(0.f);
            if (auto* pv = std::get_if<glm::vec3>(&pin.value)) v = *pv;
